...
def reconstruct(directory, z_products={}, w_products={}, xx_products={}, extra_ps={}, count_histos=[], xx_lla=''):
    """
    ...
    # Amount of Zs, Ws, and/or photons wanted per event
    zs_wanted, ws_wanted, ps_wanted = wanted_bosons(z_products, w_products, xx_products, extra_ps)
    for i in range(generated_events):
        ...
        if ('fatjet' in z_products) and ('fatjet' not in w_products):
            ...
            if fatjet_amount >= z_products['fatjet']:
                ...
                while len(z_fatjet_list) > z_products['fatjet']:
                    z_bos_deltas = [pow(abs(91.2 - z.M()), 2) for z in z_fatjet_list]
        ...
        if ('photon' in xx_products) and ('photon' not in extra_ps):
            ...
            for p in range(photon_amount):
                photons[p].SetPtEtaPhiM(tree.GetLeaf('Photon.PT').GetValue(p),
                                        ...
                                        0)
            if (photon_amount >= xx_products['photon']) and (count is True):
        ...
        if ('photon' in extra_ps) and ('photon' not in xx_products):
            photon_amount = tree.Photon.GetEntries()
            ...
            for p in range(photon_amount):
                photons[p].SetPtEtaPhiM(tree.GetLeaf('Photon.PT').GetValue(p),
                                        ...
                                        tree.GetLeaf('Photon.Phi').GetValue(p),
                                        0)
            if (photon_amount >= extra_ps['photon']) and (count is True):
                phots.append(photons)
            else:
                count = False
        else:
        ...
        if ('photon' in xx_products) and ('photon' in extra_ps):
            photon_amount = tree.Photon.GetEntries()
            ...
            for p in range(photon_amount):
                photons[p].SetPtEtaPhiM(tree.GetLeaf('Photon.PT').GetValue(p),
                                        ...
                                        tree.GetLeaf('Photon.Phi').GetValue(p),
                                        0)
            if (photon_amount >= (xx_products['photon']+extra_ps['photon'])) and (count is True):
                phots.append(photons)
            else:
                count = False
        else:
        ...
        if (count is True) and (zs_wanted == 1) and (ps_wanted == 2):
            z_list = z_muon_list + z_electron_list + z_fatjet_list
            z1_list.append(z_list[0])
            # FIXME: May need fixing due to ignoring other bosons

        if (count is True) and (ws_wanted == 1) and (ps_wanted == 2):
            w1_list.append(w_fatjet_list[0])
            # FIXME: Maybe not though

        if (count is True) and (zs_wanted == 2) and (ps_wanted == 1):
            z_list = z_muon_list + z_electron_list + z_fatjet_list
            z1_list.append(z_list[0])
            z2_list.append(z_list[1])
            # FIXME: OTHER IS THE PHOTON
        if (count is True) and (zs_wanted == 1) and (ws_wanted == 1) and (ps_wanted == 1):
            z_list = z_muon_list + z_electron_list + z_fatjet_list
            w_list = w_fatjet_list
            if (len(z_list) == 1) and (len(w_list) == 1) and ps_wanted == 1:
                z1_list.append(z_list[0])
                w1_list.append(w_list[0])
    ...
    if zs_wanted == 3:
    ...
    elif (zs_wanted == 1) and (ps_wanted == 2):
        ...
        # amount of events that made it through the selection process
        numb_events = len(xx_list)
        boson_list = [z1_list, phot1, phot2]

    elif (ws_wanted == 1) and (ps_wanted == 2):
        for k in range(len(phots)):
            if xx_products['photon'] == 2:
                xx_list.append(xx_mass(w_list=[w1_list[k]], photons=phots[k], dtype='photon', bosons='waa'))
            phot1.append(phots[k][0])
            phot2.append(phots[k][1])
        numb_events = len(xx_list)
        boson_list = [w1_list, phot1, phot2]

    elif ps_wanted == 3:
        for k in range(len(phots)):
            xx_list.append(xx_mass(photons=phots[k], dtype='photon', bosons='aaa'))
            phot1.append(phots[k][0])
            phot2.append(phots[k][1])
            phot3.append(phots[k][2])
        numb_events = len(xx_list)
        # FIRST TWO ARE XX PHOTONS
        boson_list = [phot1, phot2, phot3]

    elif (zs_wanted == 2) and (ps_wanted == 1):
        for k in range(len(z1_list)):
            if len(extra_ps) != 0:
                xx_list.append(xx_mass(z_list=[z1_list[k], z2_list[k]], photons=phots[k], dtype='pt', bosons='azz'))
                phot1.append(phots[k][0])
                boson_list = [z1_list, z2_list, phot1]
            elif len(extra_ps) == 0:
                if xx_lla == 'muon':
                    xx_list.append(
                        xx_mass(z_list=[z1_list[k]], photons=phots[k], dtype='photon', bosons='zza'))
                    phot1.append(phots[k][0])
                    boson_list = [z1_list, phot1, z2_list]
                elif xx_lla == 'electron':
                    xx_list.append(
                        xx_mass(z_list=[z2_list[k]], photons=phots[k], dtype='photon', bosons='zza'))
                    phot1.append(phots[k][0])
                    boson_list = [z1_list, phot1, z2_list]
                elif xx_lla == 'fatjet':
                    xx_list.append(
                        xx_mass(z_list=[z2_list[k]], photons=phots[k], dtype='photon', bosons='zza'))
                    phot1.append(phots[k][0])
                    boson_list = [z2_list, phot1, z1_list]
                else:
                    xx_list.append(xx_mass(z_list=[z1_list[k], z2_list[k]], photons=phots[k], dtype='photon', bosons='zza'))
                    phot1.append(phots[k][0])
                    boson_list = [z1_list, phot1, z2_list]
        numb_events = len(xx_list)
    elif (zs_wanted == 1) and (ws_wanted == 1) and (ps_wanted == 1):
        for k in range(len(z1_list)):
            xx_list.append(xx_mass(z_list=[z1_list[k]], w_list=[w1_list[k]], photons=phots[k], dtype='photon', bosons='wza'))
            phot1.append(phots[k][0])
        boson_list = [z1_list, phot1, w1_list]
        numb_events = len(xx_list)
...
def wanted_bosons(z_products, w_products, xx_products, extra_ps):
    '''
    Tell how many of each boson (Z, W, photon) will be required per event.
    Inputs are dictionaries telling us what the final products will be reconstructed to.
    '''
    ...
    for p in extra_ps:
        if p == 'photon':
            ps_wanted += extra_ps[p]

    return zs_wanted, ws_wanted, ps_wanted
