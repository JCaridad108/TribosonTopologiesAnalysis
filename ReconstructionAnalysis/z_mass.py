# MODULE FOR Z BOSON RECONSTRUCTIONS
...
def z_mass(amnt, plus=[], minus=[], neutral=[]):  # mu_plus and mu_minus might both be greater then 3
    ...
    if (len(plus) > 0) and (len(minus) > 0):
        ...
        for i in left:
            ...
            for j in right:
                if i != j:
                    deltas.append(pow(abs(91 - (i + j).M()), 2))
        ...
        while len(z_bos) > amnt:
            z_bos_deltas = [pow(abs(91 - z.M()), 2) for z in z_bos]
    ...
    if len(neutral) > 0:
        ...
        for d in neutral:
            ...
            option_mass_deltas = [pow(abs(91-k.M()), 2) for k in option_list]
