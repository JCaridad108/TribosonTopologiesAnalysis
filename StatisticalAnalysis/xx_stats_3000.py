...
if __name__ == "__main__":    
    ...
    crossx_list = [0.00000033740, 0.0000003374, 0.0000002601, 0.0000001664]
    
    #signal_hists = ['sig_content1.txt', 'sig_content2.txt', 'sig_content3.txt', 'sig_content4.txt']
    signal_hists = ['sig_content3.txt', 'sig_content4.txt']
    
    # Background histogram as list
    back_data = []
    background_file = open('bkg_bin_content.txt', 'r')
    
    for event in background_file.read().split('\n'):
        line = event.replace('\n', '')
        if len(line) > 1:
            back_data.append(float(line)*30)
